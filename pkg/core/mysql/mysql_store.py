...
class MySQLStore:
    ...
    async def get_pool(self) -> aiomysql.Pool:
        """获取数据库连接池"""
        try:
            if self.pool is None or self.pool._closed:  # 检查连接池是否关闭
        ...
        while retry_count < max_retries:
            try:
                ...
                if params:
                    ...
                    logger.info(f"Executing SQL: {actual_sql} trace_id: {trace_id}")
                else:
                    logger.info(f"Executing SQL: {query} trace_id: {trace_id}")
